(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('next/link'), require('next/router')) :
	typeof define === 'function' && define.amd ? define(['exports', 'react', 'next/link', 'next/router'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global['next-routes-dynamic'] = {}, global.React, global.OriginalNextLink, global.OriginalNextRouter));
}(this, (function (exports, React, OriginalNextLink, OriginalNextRouter) { 'use strict';

	React = React && Object.prototype.hasOwnProperty.call(React, 'default') ? React['default'] : React;
	OriginalNextLink = OriginalNextLink && Object.prototype.hasOwnProperty.call(OriginalNextLink, 'default') ? OriginalNextLink['default'] : OriginalNextLink;
	var OriginalNextRouter__default = 'default' in OriginalNextRouter ? OriginalNextRouter['default'] : OriginalNextRouter;
	...
	class Registry {
	    ...
	    createOption(name, pattern, page) {
	        let options = {};
	        let _name = name;
	    ...
	    add(name, pattern, page) {
	        const options = name instanceof Object ? name : this.createOption(name, pattern, page);
	     ...
	     * @param page
	     */
	    set(name, pattern, page) {
	        const options = name instanceof Object ? name : this.createOption(name, pattern, page);
	        const curRoute = this.findByName(options.name);
	        if (!!curRoute) {
	            if (options.pattern)
	                curRoute.updatePattern(options.pattern);
	            if (options.page)
	                curRoute.updatePage(options.page);
	        }
	        else {
	            this._routes.push(new Route(options));
	        }
	    ...
	    match(url) {
	        ...
	        return this._routes.reduce((result, route) => {
	            ...
	            if (!params)
	                return result;
	            return Object.assign(Object.assign({}, result), { route, params, query: Object.assign(Object.assign({}, query), params) });
	     ...
	     * @param customHandler
	     */
	    getRequestHandler(app, customHandler = null) {
	        const nextHandler = app.getRequestHandler();
	        return (req, res, queryHandler = null) => {
	...
	***************************************************************************** */
	...
	function useDynamicRouteMatch(registry, nameOrUrl, params = {}) {
	    ...
	    React.useEffect(() => {
	        const remover = registry.on(EVENT_CHANGE, () => {
	            const newResult = !nameOrUrl
	                ? createDefaultResult()
	                : registry.findAndGetUrls(nameOrUrl, params);
	            if (!newResult.urls || !isEqual_1(lastUrls, newResult.urls)) {
	...
	function createInjectedLink(registry, Link = null) {
	    const LinkRenderer = Link || OriginalNextLink;
	    function DynamicRouteLink(props) {
	        const { route, params, to } = props, newProps = __rest(props, ["route", "params", "to"]);
	        ...
	        return React.createElement(LinkRenderer, Object.assign({}, _newProps));
	    }
	    return DynamicRouteLink;
	}

	function createInjectedRouter(registry, Router = null) {
	    const _router = Router || OriginalNextRouter__default;
	    const _injectingRouter = Object.assign({}, _router);
	    const wrap = (method) => (route, params, options) => {
	        const _method = _router[method];
	        if (!!_method) {
	            const { byName, urls: { as, href }, } = registry.findAndGetUrls(route, params);
	...
	function createUseRouteRegistry(registry) {
	    return () => {
	        const router = OriginalNextRouter.useRouter();
	...
	const createRegistry = (options) => {
	    const { Link: LinkTemplate = null, Router: RouterTemplate = null } = options || {};
